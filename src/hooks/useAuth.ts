...
export function useAuth<User extends AuthRecord>({ collectionName = 'users' }: UseAuthOptions = {}): UseAuthResult<User> {
    ...
    () => ({
      email: async (email: string, password: string, options?: RecordOptions) => {
        try {
          setIsLoading(true);
          setError(null);
          const { record } = await recordService.authWithPassword<User>(email, password, options);
          return record;
        } catch (err) {
      ...
      social: async (provider: string, options: Omit<OAuth2AuthConfig, 'provider'> = {}) => {
        try {
          setIsLoading(true);
          setError(null);
          const { record } = await recordService.authWithOAuth2<User>({ provider, ...options });
          return record;
        } catch (err) {
          setError(err as Error);
          return null;
      ...
      otp: async (otp: string, password: string, options?: RecordOptions) => {
        try {
          setIsLoading(true);
          setError(null);
          const { record } = await recordService.authWithOTP<User>(otp, password, options);
          return record;
        } catch (err) {
          setError(err as Error);
          return null;
  ...
  const refreshAuth = useCallback(async () => {
    const { record } = await recordService.authRefresh<User>();
    setUser(record);
  }, [recordService]);
