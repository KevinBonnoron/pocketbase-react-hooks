  ...
  const create = useCallback(
    async (bodyParams: Partial<Record>, options?: RecordOptions): Promise<Record | null> => {
      try {
        setIsPending(true);
        setError(null);
        const record = await pb.collection(collectionName).create(bodyParams, options);
  ...
  const update = useCallback(
    async (id: string, bodyParams: Partial<Record>, options?: RecordOptions): Promise<Record | null> => {
      try {
        setIsPending(true);
        setError(null);
        const record = options ? await pb.collection(collectionName).update(id, bodyParams, options) : await pb.collection(collectionName).update(id, bodyParams);
        return record as Record;
      } catch (err) {
