...
- `signIn`: Object containing authentication methods
  - `signIn.email(email, password)`: Sign in with email and password
  - `signIn.social(provider)`: Sign in with OAuth2 provider (e.g., 'google', 'github')
- `signUp`: Object containing registration methods
  - `signUp.email(email, password, data?)`: Create new user account
