...
- `useRecord` automatically updates when the specific record changes (can be disabled with `realtime: false`)
- `useAuth` automatically updates when authentication state changes

**Note:** Real-time subscriptions are enabled by default but can be disabled using the `realtime: false` option for better performance when real-time updates are not needed.
