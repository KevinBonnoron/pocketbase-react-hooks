...
- `signIn`: Object containing authentication methods
  ...
  - `signIn.social(options)`: Sign in with OAuth2 provider using configuration object
...
The `signIn.social()` method accepts an `OAuth2AuthConfig` object with the following options:

```typescript
interface OAuth2AuthConfig {
  provider: string;                    // Required: OAuth2 provider name (e.g., 'google', 'github', 'discord')
